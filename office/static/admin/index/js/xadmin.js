;!function (win) {
     ...
     * @return {[type]}        [description]
     */
    Xadmin.prototype.open = function (title, url, w, h, full = false,shade=0) {
        if (title == null || title == '') {
            var title = false;
        }
        ;
...
layui.use(['layer', 'element', 'jquery'], function () {
    ...
    $(".layui-tab-title").on('dblclick', 'li', function (event) {
        ...
        var src=$(this).parent().parent().find("iframe[tab-id="+lay_id+"]").attr("src")
        $(this).parent().parent().find("iframe[tab-id="+lay_id+"]").attr("src",src);
        return false;
    });
    ...
    $('#tab_right').on('click', 'dd', function (event) {
        ...
        if(data_type == 'refresh'){
            var src=$(this).parents('#tab_right').next().find("iframe[tab-id="+lay_id+"]").attr("src")
            $(this).parents('#tab_right').next().find("iframe[tab-id="+lay_id+"]").attr("src",src);
        }else if (data_type == 'this') {

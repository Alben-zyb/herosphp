...
layui.define(['laytpl', 'form', 'util'], function (exports) {
    ...
    /** LevelTable类构造方法 */
    var LevelTable = function (options) {
        _instances[options.elem.substring(1)] = this;
        this.reload(options);
    };

    /** 参数设置 */
    LevelTable.prototype.initOptions = function (opt) {
        var that = this;
    ...
    /** 初始化表格 */
    LevelTable.prototype.init = function () {
    ...
    /** 绑定各项事件 */
    LevelTable.prototype.bindEvents = function () {
        var that = this;
        var options = this.options;
        var components = this.getComponents();
        ...
        var member = function (ext) {
            ...
            var obj = {
                ...
                refresh: function () {  // 刷新子表(我添加的功能)
                    that.renderBodyAsync(this.data, $tr);
                },
            };
        ...
        $allBody.off('click.fold').on('click.fold', '.ew-tree-pack', function (e) {
            ...
            /*我的:begin*/
            //只允许一个展开
            $tr.addClass('isClick');  //添加当前点击标记
            var $otherTrs = $(this).parentsUntil('tr').last().parent().parent().find('tr');
            $.each($otherTrs, function (index, val) {
                var $otherTr = $(val).last();
                var open = $otherTr.hasClass('ew-tree-table-open');
                var isClick = $otherTr.hasClass('isClick');
                var hide = $otherTr.hasClass('ew-tree-tb-hide');
                var data = that.getDataByTr($otherTr);
                if (!hide && !isClick && open && data[options.tree.childName]) {
                    data[options.tree.openName] = toggleRow($otherTr);
                }
            })
            $tr.removeClass('isClick'); //移除当前点击标记
            /*我的:end*/
    ...
    /** 获取各个组件 */
    LevelTable.prototype.getComponents = function () {
     ...
     * @param obj
     */
    LevelTable.prototype.eachCols = function (callback, obj) {
     ...
     * @param data
     */
    LevelTable.prototype.eachData = function (callback, data) {
     ...
     * @param $tr 父级dom
     */
    LevelTable.prototype.renderBodyAsync = function (d, $tr) {
        var that = this;
        var options = this.options;
        var components = this.getComponents();
        // 显示loading
     ...
     * @param $tr 父级dom
     */
    LevelTable.prototype.renderBodyData = function (data, d, $tr) {
        var msg;
     ...
     * @returns {string}
     */
    LevelTable.prototype.renderBody = function (data, indent, parent, h) {
        var options = this.options;
        if (!indent) indent = 0;
        var html = '';
     ...
     * @returns {string}
     */
    LevelTable.prototype.renderBodyTr = function (d, indent, hide, $tr) {
        var that = this;
        var options = this.options;
        if (!indent) indent = 0;
     ...
     * @returns {string}
     */
    LevelTable.prototype.renderBodyTd = function (d, indent, index, $td, col) {
        if (col.colGroup) return '';
        var options = this.options;
        var components = this.getComponents();
        if (!indent) indent = 0;
        ...
        if (index === options.tree.iconIndex) {
            ...
            //var haveChild = d[options.tree.haveChildName];
            //我的修改
            var haveChild = d[options.tree.haveChildName]&&(d[options.tree.haveChildName]==='true'||d[options.tree.haveChildName]===true);
            if (haveChild === undefined) haveChild = d[options.tree.childName] && d[options.tree.childName].length > 0;
        ...
        cell = [
            ...
            /*'   <div class="layui-table-grid-down" style="display: none;"><i class="layui-icon layui-icon-down"></i></div>,'我修改的地方*/
            '</div>'
        ].join('');
     ...
     * @returns {string}
     */
    LevelTable.prototype.renderBodyTh = function () {
        var options = this.options;
        var components = this.getComponents();
        var html = [];
        $.each(options.cols, function (i1, item1) {
            ...
            $.each(item1, function (i2, item2) {
                ...
                //html.push('<div class="layui-table-grid-down" style="display: none;"><i class="layui-icon layui-icon-down"></i></div></div>');//我修改的地方
                // 列宽拖拽
    ...
    /** 重置表格尺寸 */
    LevelTable.prototype.resize = function (returnColgroup) {
        // 计算表格宽度、最小宽度、百分比宽度
        var options = this.options;
        var components = this.getComponents();
    ...
    /** 获取行对应数据 */
    LevelTable.prototype.getDataByTr = function ($tr) {
        var data, index;
        ...
        for (var i = 0; i < index.length; i++) {
            if (data&&(typeof (data[this.options.tree.childName]) != 'undefined')) data = data[this.options.tree.childName][index[i]];
     ...
     * @param checked
     */
    LevelTable.prototype.checkSubCB = function ($tr, checked) {
        var that = this;
        var components = this.getComponents();
        var indent = -1, $trList;
        if ($tr.is('tbody')) {
     ...
     * @param $tr 父级的dom
     */
    LevelTable.prototype.checkParentCB = function ($tr) {
        var options = this.options;
        var components = this.getComponents();
    ...
    /** 联动全选复选框 */
    LevelTable.prototype.checkChooseAllCB = function () {
        var options = this.options;
        var components = this.getComponents();
        var ckNum = 0, unCkNum = 0;
    ...
    /** 填充序号列 */
    LevelTable.prototype.renderNumberCol = function () {
    ...
    /** 根据id获取tr的index */
    LevelTable.prototype.getIndexById = function (id) {
        var options = this.options;
    ...
    /** 展开指定行 */
    LevelTable.prototype.expand = function (id, cascade) {
        var components = this.getComponents();
    ...
    /** 折叠指定行 */
    LevelTable.prototype.fold = function (id) {
        var components = this.getComponents();
        var $tr = components.$table.children('tbody').children('tr[data-index="' + this.getIndexById(id) + '"]');
        if ($tr.hasClass('ew-tree-table-open')) $tr.children('td').find('.ew-tree-pack').trigger('click');
    };
    ...
    /** 全部展开 */
    LevelTable.prototype.expandAll = function () {
    ...
    /** 获取是否有展开 */
    LevelTable.prototype.isExpand = function () {
        var flag = false;
        this.getComponents().$table.children('tbody').children('tr').each(function () {
            if ($(this).hasClass('ew-tree-table-open')) {
                flag = true;
    ...
    /** 全部折叠 */
    LevelTable.prototype.foldAll = function () {
        this.getComponents().$table.children('tbody').children('tr').each(function () {
            if ($(this).hasClass('ew-tree-table-open')) $(this).children('td').find('.ew-tree-pack').trigger('click');
        });
    };
    ...
    /** 获取当前数据 */
    LevelTable.prototype.getData = function () {
        return this.options.data;
    };

    /** 重载表格 */
    LevelTable.prototype.reload = function (opt) {
    ...
    /** 获取当前选中行 */
    LevelTable.prototype.checkStatus = function (needIndeterminate) {
    ...
    /** 设置复/单选框选中 */
    LevelTable.prototype.setChecked = function (ids) {
        var that = this;
        var components = this.getComponents();
    ...
    /** 移除全部选中 */
    LevelTable.prototype.removeAllChecked = function () {
        this.checkSubCB(this.getComponents().$table.children('tbody'), false);
    };

    /** 导出 */
    LevelTable.prototype.exportData = function (type) {
        var components = this.getComponents();
        if ('show' === type) {
    ...
    /** 打印 */
    LevelTable.prototype.printTable = function () {
        var components = this.getComponents();
    ...
    /** 筛选列 */
    LevelTable.prototype.toggleCol = function (show, field, key) {
        var components = this.getComponents();
        if (show === undefined) {
            components.$toolbar.find('.layui-table-tool-panel').remove();
     ...
     */
    LevelTable.prototype.filterData = function (ids) {
        var components = this.getComponents();
        components.$loading.show();
    ...
    /** 重置搜索 */
    LevelTable.prototype.clearFilter = function () {
        var components = this.getComponents();
     ...
     */
    LevelTable.prototype.refresh = function (id, data) {
        if (isClass(id) === 'Array') {
            data = id;
            id = undefined;
        }
    ...
    /** 删除数据 */
    LevelTable.prototype.del = function (id, index) {
    ...
    /** 更新数据 */
    LevelTable.prototype.update = function (id, fields) {

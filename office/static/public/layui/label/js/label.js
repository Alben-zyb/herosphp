...
//添加自定义标签
function addLabelInterface(id, name) {
    // 3情况：重复、标签库、新增
    var labelId = id;
    var labelName = name;
    ...
    if (labelId != null && labelId != "") {
        ...
        } else if (indexOfexisted !== -1) //已经存在于标签库，添加，并要把标签库里相应的标签设置为已选择
        {
            ...
            if (addLabel(labelId, labelName))   //新添加的标签，添加，并2+
            {
...
function getLabelIds() {
    var labelIds = $("input[name='imagelabels']").val();
    if(!labelIds){
        return new Array();
    }
    var ids=(labelIds.substr(0,labelIds.length-1)).split(',');
    return ids;
}

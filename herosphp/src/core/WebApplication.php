...
class WebApplication implements IApplication {
    ...
    private function __construct() {
        ...
        if (file_exists(APP_PATH."modules/DefaultWebappListener.php")) {
            $listenerClassName = APP_NAME."\\DefaultWebappListener";
            try {
                $reflect = new \ReflectionClass($listenerClassName);
    ...
    public function execute($configs) {
        ...
        $listenerClassName = APP_NAME."\\".$this->httpRequest->getModule()."\\ModuleListener";
        try {
            $reflect = new \ReflectionClass($listenerClassName);
            $this->listeners[] = $reflect->newInstance();
        } catch (\Exception $exception) {
